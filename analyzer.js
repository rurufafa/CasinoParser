export default class GambleAnalyzer {
    ...
    _analyzeBarLog(ids) {
        const stats = {
            ...
            // genre.bars = { name: { pay, payAmount, gain, gainAmount, probability, dangoRank: {} } }
            loseStreaks : {}, // { barName: [count, ...] }
            winStreaks : {},  // { barName: [count, ...] }
        };
        ...
        for (const id of ids) {
            ...
            if (direction === "lose") {
                ...
                for (const bar in lastLoseCount) {
                    ...
                    // 連勝記録が0なら記録しない
                    if (lastLoseCount[bar] > 0)
                        stats.winStreaks[bar].push(lastLoseCount[bar]);
                    lastLoseCount[bar] = 0;
                }
            ...
            if (direction === "gain") {
                ...
                // 団子酒のとき金額ごとにも保持する
                if (genre === "Dango") {
                    if (!b.dangoRank[amount])
                        b.dangoRank[amount] = 0;
                    b.dangoRank[amount] += amount;
                }
                ...
                if (lastGainCount[name]) {
                    ...
                    stats.loseStreaks[name].push(lastGainCount[name]);
                    lastGainCount[name] = 0;
                }
        ...
        for (const bar in lastGainCount) {
            if (lastGainCount[bar]) {
                ...
                stats.loseStreaks[bar].push(lastGainCount[bar]);
            }
        }
        for (const bar in lastLoseCount) {
    ...
    _analyzeSlotLog(ids) {
        const stats = {
            ...
            prices : {}, // { price: { payAmount, gainAmount, slots: { name: { payAmount,gainAmount } } } }
        };

        for (const id of ids) {
            const log = this.logs[id];

            const {name, price, amount, direction} = log;
            if (direction === "hint")
                continue;

            if (!name || !(name === "不明" || (name in this.slotInfo)))
                continue;
